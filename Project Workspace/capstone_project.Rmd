...
XGB_Caret <- train(x = train.set, y = df.combined$SalePrice[!is.na(df.combined$SalePrice)], method = 'xgbTree', trControl = Control.Train, tuneGrid = XGB_Grid)
XGB_Caret$bestTune
```
...
train.set.lm <- df.combined.predictors[!is.na(df.combined$SalePrice), ]
test.set.lm <- df.combined.predictors[is.na(df.combined$SalePrice),] 
Features.Lm <- names(df.combined.predictors)
Features.Lm <- paste("x", 1:182, sep = "")
Formula <- as.formula(paste("df.combined$SalePrice ~", paste(Features.Lm, collapse = "+")))

Model.Lm <- lm(formula, data = df.combined.predictors)

Model.Lm <- lm(df.combined$SalePrice ~ GrLivArea + Neighborhood + OverallQual + MSSubClass + BsmtFinType1 + GarageType + ExterQual + FireplaceQu + Exterior2nd + GarageFinish + CentralAir, data = train.set.lm)
summary(model.lm)
